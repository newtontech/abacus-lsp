...
from abacus_lsp.agent_lsp import AgentLSP
from abacus_lsp.rich_diagnostics import (
...
def test_legacy_diagnostic_is_enriched() -> None:
    ...
    item = diagnostic_to_dict(diagnostic, software="abacus", path="input")
...
def test_rich_diagnostics_handles_dataclass_json_and_lsp_ranges() -> None:
    dataclass_item = diagnostic_to_dict(
        _DataclassDiagnostic(
            code="ABACUSDATA",
            severity=2,
            message="scf runtime risk",
            line=4,
            column=5,
            suggested_fix="tighten convergence settings",
        ),
        software="abacus",
        path="INPUT",
        file_type="input",
    ...
    assert dataclass_item["fix_hints"] == ["tighten convergence settings"]

    json_item = diagnostic_to_dict(_JsonDiagnostic(), software="abacus")
    ...
    fallback_item = diagnostic_to_dict(_BrokenJsonDiagnostic(), software="abacus")
...
def test_agent_lsp_text_path_and_operations(
    ...
    monkeypatch.setattr("abacus_lsp.agent_lsp.check_path", fake_check_path)

    agent = AgentLSP.from_text("INPUT_PARAMETERS\ncalculation scf\n")
    payload = agent.check()
...
def test_tool_main_emits_json_and_fail_on_blocking(
    ...
    assert payload["diagnostics"][0]["file_type"] == "input"
...
""",
    ...
    assert isinstance(tool._collect_diagnostics(tmp_path), list)
    assert tool._file_type(tmp_path / "INPUT") == "input"
